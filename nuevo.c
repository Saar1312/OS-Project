...
void Map(char fileName, Persona *p, FILE *f,char *str1,char *str2,Persona *amigo);
...
void eliminarLinea(char nombre,FILE *archivo,int numlinea,char *str1,char *str2,Persona *amigo, char *linea){
...
int buscarPar(char fileName, FILE *archivo,char *persona1,char *persona2,Persona *amigo){
			...
			while ((caracter = fgetc(f)) != EOF){	
				if ((i*lineas_proceso <= contador) && (contador < (i+1)*lineas_proceso)){
					...
					if (p->amigos->nombre != "-None-"){
						Map((char)getpid(),p,f,p->nombre,p->amigos->nombre,p->amigos);
					}
				}
				if (caracter == '\n'){
					contador++;
...
void Map(char fileName,Persona *p, FILE *f,char *str1,char *str2,Persona *amigo){
	...
	if (!buscarPar(fileName,p,f,str1,str2,amigo)){
